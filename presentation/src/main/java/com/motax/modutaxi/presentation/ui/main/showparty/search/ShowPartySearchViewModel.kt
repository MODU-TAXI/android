...
@HiltViewModel
class ShowPartySearchViewModel @Inject constructor(
    private val repository: NaverRepository
) : ViewModel() {

    private val _event = MutableSharedFlow<ShowPartySearchEvent>()
    val event: SharedFlow<ShowPartySearchEvent> = _event.asSharedFlow()

    private val _uiState = MutableStateFlow(ShowPartySearchUiState())
    val uiState: StateFlow<ShowPartySearchUiState> = _uiState.asStateFlow()
    ...
    private fun selectLocation(latitude: Double, longitude: Double, landMark: String, address: String) {
        viewModelScope.launch {
            _event.emit(ShowPartySearchEvent.SelectLocation(latitude, longitude, landMark))
        }
    }

    fun navigateToBack() {
        viewModelScope.launch {
            _event.emit(ShowPartySearchEvent.NavigateToBack)
        }
    }
